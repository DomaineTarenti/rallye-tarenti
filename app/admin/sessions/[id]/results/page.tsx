...
export default function ResultsPage() {
  ...
  async function exportPdf() {
    ...
    try {
      ...
      doc.text(session?.name ?? "The Quest", 105, 20, { align: "center" });

      doc.setFontSize(12);
      doc.setFont("helvetica", "normal");
      doc.text("Final Results", 105, 28, { align: "center" });
      ...
      sorted.forEach((team, i) => {
        ...
        doc.text(team.name, 25, y);
        doc.text(String(score), 110, y);
        doc.text(rank.label, 135, y);
        doc.text(`${team.completed_steps}/${team.total_steps}`, 160, y);
        doc.text(formatDuration(team.elapsed_seconds), 185, y);
        y += 7;
      });
      ...
      doc.text(`Generated by The Quest — ${new Date().toLocaleDateString()}`, 105, 290, { align: "center" });

      doc.save(`${session?.name ?? "quest"}-results.pdf`);
    } catch (err) {
          ...
          {sorted.length === 0 ? (
            ...
            <div className="divide-y divide-gray-50">
              {sorted.map((team, i) => {
                ...
                const rank = getRank(score);
                const char = parseCharacter(team.character);
                const colors = rank.key ? RANK_COLORS[rank.key] : null;
