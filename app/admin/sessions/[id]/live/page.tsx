...
export default function LiveDashboard() {
  ...
  async function unlockStep(teamId: string) {
    ...
    try {
      // Find the team's active progress and complete it, then activate next
  ...
  async function emergencyStop() {
    if (!confirm("Are you sure you want to stop this session? All teams will be paused.")) return;
  ...
  // Sort by progress (most completed first), then by elapsed time
        ...
        <div className="mb-6 flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-0.5 text-sm text-gray-500">
              {session?.name} · {playingTeams.length} playing · {finishedTeams.length} finished
            </p>
          </div>
                      ...
                      <tr key={team.id} className={`border-b border-gray-50 ${isStuck ? "bg-amber-50/50" : ""}`}>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              ...
                              {char?.teamCode && (
                                <p className="font-mono text-[10px] text-gray-400">{char.teamCode}</p>
                              )}
                            </div>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          <span className="text-gray-700">
                            {team.completed_steps}/{team.total_steps}
                          </span>
                          <div className="mt-1 h-1 w-16 overflow-hidden rounded-full bg-gray-100">
                            <div
                              className="h-full rounded-full bg-indigo-500"
                              style={{ width: `${team.total_steps > 0 ? (team.completed_steps / team.total_steps) * 100 : 0}%` }}
                            />
                          </div>
                        </td>
                        ...
                        <td className="px-4 py-3">
                          <span className={`rounded px-2 py-0.5 text-[10px] font-semibold ${
                            ...
                            {isStuck ? "Stuck" : team.status}
                          </span>
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex gap-1">
                            <button
                              onClick={() => unlockStep(team.id)}
                              disabled={team.status !== "playing" || actionLoading === `unlock-${team.id}`}
                              title="Unlock current step"
                              className="rounded-lg p-1.5 text-gray-400 hover:bg-gray-100 hover:text-indigo-600 disabled:opacity-30"
                            >
                              {actionLoading === `unlock-${team.id}` ? (
